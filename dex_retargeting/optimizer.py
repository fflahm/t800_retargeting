...
svh_link_names = ["right_hand_base_link",
...
class PositionOptimizer(Optimizer):
    ...
    def get_objective_function(self, target_pos: np.ndarray, fixed_qpos: np.ndarray, last_qpos: np.ndarray):
        ...
        def objective(x: np.ndarray, grad: np.ndarray) -> float:
            ...
            result = result + self.norm_delta * np.dot(x - last_qpos, x - last_qpos) # Not sure whether it is right
...
class VectorOptimizer(Optimizer):
    ...
    def get_objective_function(self, target_vector: np.ndarray, fixed_qpos: np.ndarray, last_qpos: np.ndarray):
        ...
        torch_target_vec = torch.as_tensor(target_vector) # [10,3]

        torch_target_vec[:,0] *= self.scaling[0]
        torch_target_vec[:,1] *= self.scaling[1]
        torch_target_vec[:,2] *= self.scaling[2]
